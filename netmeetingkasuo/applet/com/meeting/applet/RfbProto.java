...
class RfbProto {
	...
	RfbProto(String h, int p, TightVncViewer v) throws IOException {
		viewer = v;
		host = h;
		port = p;
		UseTLS = v.sslUse;
		...
		// ORMitchell - 08/06/2003
		if (UseTLS == 1) { // If you we want to the TLS/SSL encapsulation
			// then...
			try {
				...
				if (viewer.inAnApplet) // Need to understand the significance of
				// the "viewer" and "viewer.mainArgs"
				// stuff !!!!

				// ORMitchell - 12/03/2003
				// Attempt to allow the JAVA SSL code to support a non-CA signed
				// or self-signed certificate...

				{
					socket = (SSLSocket) factory.createSocket(host, port);
					// socket.setAllowUserInteraction(true);
				}
			...
			if (viewer.socketFactory == null) { // If an alternate
				// "SocketFactory" is defined,
				// then use it ????
				...
				try {
					...
					if (viewer.inAnApplet)
						...
						sock = factory
								.createSocket(host, port, viewer.mainArgs);
				} catch (Exception e) {
					e.printStackTrace();
					throw new IOException(e.getMessage());
				}
	...
	// Support Function - ripped from other version
	...
	void writeClientInit() throws IOException {
		if (viewer.buttonPanel.shareDesktop) {
			os.write(1);
		} else {
			os.write(0);
		}
		viewer.buttonPanel.shareDesktop = false;
	}
	...
	void writePointerEvent(MouseEvent evt) throws IOException {
		...
		int mask2 = 2;
		int mask3 = 4;
		if (viewer.buttonPanel.reverseMouseButtons2And3) {
			mask2 = 4;
			mask3 = 2;
		}
