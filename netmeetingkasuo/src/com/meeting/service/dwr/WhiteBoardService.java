...
public class WhiteBoardService extends DWRService {

	private static Logger logger = Logger.getLogger(WhiteBoardService.class);
	...
	public static String WHITEBOARD_FOLDER = null;
	...
	public void startService() throws Exception {
		try {
			...
			logger.info("加载白板面板...会议：" + meetingId + ", 用户："
					+ curuser.getUsername());
			...
			for (String tmpWdId : wbIdSet) {
				String newImgageName = getImageName(tmpWdId);

				SessionCall(ssession, "whiteBoardInitCallback", tmpWdId,
						newImgageName, responseText(tmpWdId));
			}
		} catch (Exception e) {
			logger.error("启动白板异常：" + StackTraceUtil.getStackTrace(e));
			throw new WhiteBoardServiceException("启动白板异常：" + e);
		}
	}
	 ...
	 * @throws Exception
	 */
	public void createWhiteBoard(String wbId) throws Exception {
		try {
			logger.info("创建白板..." + wbId);
			String meetingId = meeting.getMeetingId();
			String imageName = System.currentTimeMillis() + ".gif";
			SessionsCall(meetingId, "whiteBoardInitCallback", wbId, imageName);
		} catch (Exception e) {
			logger.error("创建白板异常：" + StackTraceUtil.getStackTrace(e));
			throw new WhiteBoardServiceException("创建白板异常：" + e);
		}
	}
	...
	public void createWhiteBoardContent(String wbId, String imageName,
		...
		try {
			logger.info("初始化白板内容图片：文件名" + imageName + ", 宽：" + width + ", 高："
					+ height);
			String meetingId = meeting.getMeetingId();
			Map<String, String> wbMap = mWbMap.get(meetingId);
			wbMap.put(wbId, wbId);
			...
			if (WHITEBOARD_FOLDER == null || WHITEBOARD_FOLDER.equals("")) {
				WHITEBOARD_FOLDER = AppConfigure.upload_path + "/" + meetingId
						+ "/whiteboard";
			}
			String folderPath = WHITEBOARD_FOLDER + "/" + wbId;
			...
			logger.error("初始化白板内容图片异常：" + StackTraceUtil.getStackTrace(e));
			throw new WhiteBoardServiceException("初始化白板内容图片异常：" + e);
		}
	}
	...
	public void drawPolyLine(String color, String wbId, String newImgageName,
		...
		try {
			...
			String meetingId = meeting.getMeetingId();
			String folderPath = WHITEBOARD_FOLDER + "/" + wbId;
			File srcFile = new File(folderPath + "/" + getImageName(wbId));
			...
			throw new WhiteBoardServiceException("画曲线异常：" + e);
		}
	}
	...
	public void drawLine(String color, String wbId, String newImgageName,
		...
		try {
			...
			throw new WhiteBoardServiceException("画直线段异常：" + e);
		}
	}
	...
	public void drawArrowLine(String color, String wbId, String newImgageName,
		...
		try {
			...
			throw new WhiteBoardServiceException("画箭头线段异常：" + e);
		}
	}
	...
	public void drawText(String wbId, String msg, String x, String y,
		...
		try {
			...
			SessionsCall(meetingId, "whiteBoardDrawTextCallback", wbId, msg, x,
					y, color);
		} catch (Exception e) {
			logger.error("写文字异常：" + StackTraceUtil.getStackTrace(e));
			throw new WhiteBoardServiceException("写文字异常：" + e);
		}
	}
	...
	public void drawRectangle(String color, String wbId, String newImgageName,
		...
		try {
			...
			throw new WhiteBoardServiceException("画矩形异常：" + e);
		}
	}
	...
	public void drawEllipse(String color, String wbId, String newImgageName,
		...
		try {
			...
			throw new WhiteBoardServiceException("画椭圆异常：" + e);
		}
	}
	...
	public void whiteboardDelete(String wbId) throws Exception {
		try {
			...
			throw new WhiteBoardServiceException("删除白板异常：" + e);
		}
	}
	...
	public void drawUndo(String wbId) throws Exception {
		try {
			...
			throw new WhiteBoardServiceException("返回上一步骤白板异常：" + e);
		}
	}
	...
	public void drawRedo(String wbId) throws Exception {
		try {
			...
			throw new WhiteBoardServiceException("返回重做步骤白板异常：" + e);
		}
	}
	...
	public void drawClear(String wbId, String imageName, String width,
		...
		try {
			logger.info("清除白板：新文件名" + imageName);
			String meetingId = meeting.getMeetingId();
			Map<String, String> wbMap = mWbMap.get(meetingId);
			wbMap.remove(wbId);
			mWbMap.put(meetingId, wbMap);
			...
			logger.error("清除白板异常：" + StackTraceUtil.getStackTrace(e));
			throw new WhiteBoardServiceException("清除白板异常：" + e);
		}
	}
	...
	public void enableWhiteBoard(String wbId, String flag) throws Exception {
		try {
			...
			throw new WhiteBoardServiceException("开启/关闭白板异常：" + e);
		}
	}
	 ...
	 * @throws Exception
	 */
	public String getImageName(String wbId) throws Exception {
		try {
			String meetingId = meeting.getMeetingId();
			Map<String, Integer> wbPosMap = mPosMap.get(meetingId);
			...
			throw new WhiteBoardServiceException("获取当前图片异常：" + e);
		}
	}
	 ...
	 * 将已有的文本信息重写到浏览器
	 */
	private String responseText(String wbId) throws Exception {
		try {
			String meetingId = meeting.getMeetingId();
			...
			throw new WhiteBoardServiceException("重写文字异常：" + e);
		}
	}
	 ...
	 * @return
	 */
	private int getMaxPosition(String wbId) {
		String meetingId = meeting.getMeetingId();
	 ...
	 * @param type
	 */
	private void whiteBoardDrawHandler(String meetingId, String wbId,
			String imageName, int type) {
		Integer tmpPos = getMaxPosition(wbId);
		tmpPos++;
