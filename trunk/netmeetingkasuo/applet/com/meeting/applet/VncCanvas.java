...
class VncCanvas extends Canvas implements KeyListener, MouseListener,
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7572759964749993542L;
	...
	VncCanvas(TightVncViewer v) throws IOException {
		viewer = v;
		rfb = viewer.rfb;
		...
		// changed section for automated login system
		if (viewer.autoUsername == null) {
			if (!viewer.buttonPanel.viewOnly)
				enableInput(true);
			...
			// spawn a thread to wait viewer.autoDelay then enable the listeners
			// and send the input to login
			System.out.println("Debug:  Automated login as "
					+ viewer.autoUsername);
			new AutoLogin(viewer).start();
		}
	}
	...
	public void setPixelFormat() throws IOException {
		if (viewer.buttonPanel.eightBitColors) {
	...
	void resizeDesktopFrame() {
		...
		viewer.desktopScrollPane.setSize(rfb.framebufferWidth + 2
				* Math.min(insets.left, insets.right), rfb.framebufferHeight
	...
	public void processNormalProtocol() throws Exception {
		...
		// viewer.checkRecordingStatus(); disabled
		...
		while (true) {
			...
			case RfbProto.FramebufferUpdate:
				...
				// recording features are disabled
				...
				if (viewer.deferUpdateRequests > 0 && rfb.is.available() == 0
						&& !cursorPosReceived) {
					synchronized (rfb) {
						try {
				...
				if (viewer.buttonPanel.eightBitColors != (bytesPixel == 1)) {
					setPixelFormat();
			...
			case RfbProto.ServerCutText:
				// removed clipboard integration block
				break;
	...
	void handleTightRect(int x, int y, int w, int h) throws Exception {
		...
		if (dataSize < rfb.TightMinToCompress) {
			...
			if (numColors != 0) {
				...
				if (bytesPixel == 1) {
					for (int dy = y; dy < y + h; dy++) {
						rfb.is.readFully(pixels8,
								dy * rfb.framebufferWidth + x, w);
						if (rfb.rec != null) {
	...
	public void processLocalKeyEvent(KeyEvent evt) {
		if (viewer.rfb != null && rfb.inNormalProtocol && !viewer.viewOnly) {
			if (!inputEnabled) {
	...
	synchronized void handleCursorShapeUpdate(int encodingType, int xhot,
		...
		if (viewer.buttonPanel.ignoreCursorUpdates) {
		...
		softCursorSource = new MemoryImageSource(width, height,
	...
	synchronized void softCursorMove(int x, int y) {
		if (showSoftCursor) {
			repaint(viewer.deferCursorUpdates, cursorX - hotX, cursorY - hotY,
					cursorWidth, cursorHeight);
			repaint(viewer.deferCursorUpdates, x - hotX, y - hotY, cursorWidth,
					cursorHeight);
		}
